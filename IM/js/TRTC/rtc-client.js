...
class RtcClient {
  ...
  async join() {
    ...
    try {
      ...
      layer.msg('视频通话加载错误！请检查网络！',{icon:2});
      return;
    }
    ...
    this.localStream_.play('layer_videoCall_thumb');
  ...
  async publish() {
    if (!this.isJoined_) {
      layer.msg('请先加入房间再点击开始推流！',{icon:2});
      console.warn('publish() - please join() firstly');
      return;
    }
    if (this.isPublished_) {
      console.warn('duplicate RtcClient.publish() observed');
      layer.msg('当前正在推流！',{icon:2});
      return;
    }
    try {
      // 发布本地流
  ...
  async unpublish(isLeaving) {
    if (!this.isJoined_) {
      ...
      layer.msg('请先加入房间再停止推流！',{icon:2});
      return;
    }
    if (!this.isPublished_) {
      console.warn('RtcClient.unpublish() called but not published yet');
      layer.msg('当前尚未发布本地流！',{icon:2});
      return;
    }
  ...
  handleEvents() {
    ...
    this.client_.on('peer-join', evt => {
      ...
      layer.msg('远端用户进房 - ' + userId);
    });
    // 远端用户退房通知 - 仅限主动推流用户
    this.client_.on('peer-leave', evt => {
      const userId = evt.userId;
      // console.log('peer-leave ' + userId);
      layer.msg(' 对方挂断了视频- ' + userId);
    ...
    this.client_.on('stream-subscribed', evt => {
      ...
      layer.msg('远端流订阅成功 - ' + remoteStream.getUserId());
    });
    ...
    this.client_.on('stream-removed', evt => {
      ...
      layer.msg('对方已挂断 - ' + remoteStream.getUserId());
    });
